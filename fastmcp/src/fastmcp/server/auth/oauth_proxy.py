...
class OAuthProxy(OAuthProvider):
    ...
    async def exchange_authorization_code(
        ...
        # Use refresh token TTL instead of access token TTL to prevent premature deletion
        # The refresh token may be valid for 30 days while access token expires in 1 hour
        storage_ttl = refresh_expires_in or expires_in  # Fallback to access token if no refresh token
    ...
    async def exchange_refresh_token(
        ...
        # Use refresh token TTL instead of access token TTL to prevent premature deletion
        # The refresh token may be valid for 30 days while access token expires in 1 hour
        storage_ttl = new_refresh_expires_in or 60 * 60 * 24 * 30  # Fallback to 30 days
        await self._upstream_token_store.put(
